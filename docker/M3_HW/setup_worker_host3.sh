...
echo "### Try to join '$THIRD_NODE_IP' as a worker to the swarm cluster $FIRST_NODE_IP..."
if [ -f "$CLUSTER_TOKEN_FILE" ]; then
    ...
    docker swarm join --token $CLUSTER_TOKEN --advertise-addr $THIRD_NODE_IP $FIRST_NODE_IP:$CLUSTER_PORT
else 
